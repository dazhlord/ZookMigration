...
describe("Lottery", async () => {
  ...
  beforeEach(async () => {
    ...
    const pool1 = await ethers.getContractFactory("StakingPool");
    Pool1 = await upgrades.deployProxy(
      pool1,
      [TokenV2.address, lockPeriod, UniswapRouter.address],
  ...
  describe("stake", async () => {
    it("user1 stake - new staker", async () => {
      ...
      await TokenV2.connect(user1).approve(
        Pool1.address,
    ...
    it("user1 stake again", async () => {
      ...
      expect(
        Number(rewardToClaimUser1.sub(expectReward)),
      ).to.be.lessThanOrEqual(Number(rewardToClaimUser1.div(10000)));
    });
    ...
    it("rewards are distributed with shares", async () => {
      ...
      await TokenV2.connect(user2).approve(
        Pool1.address,
      ...
      expect(
        Number(rewardToClaimUser2.sub(rewardToClaimUser1.mul(2))),
      ).to.be.lessThanOrEqual(Number(rewardToClaimUser1.div(10000)));
      expect(
        Number(rewardToClaimUser2Again.sub(rewardToClaimUser1Again)),
      ).to.be.lessThanOrEqual(Number(rewardToClaimUser1Again.div(10000)));
    });
    it("stakeForWallets", async () => {
      ...
      await Pool1.connect(user1).stakeForWallets(
        [user2.address],
        [ethers.utils.parseEther("10")],
      );
    ...
    it("stakeForWallets revert if invalid length input", async () => {
      await expect(
        Pool1.connect(user1).stakeForWallets(
          [user1.address, user2.address],
          [ethers.utils.parseEther("10")],
        ),
      ).to.be.revertedWith("INSYNC");
    });
  });
  describe("deposit reward", async () => {
    it("deposit rewards", async () => {
      ...
      expect(rewardsPerShare).to.be.eq(
        ethers.utils.parseUnits("10", 36).div(totalSharesDeposited),
      );
    });
    it("revert if invalid input amount", async () => {
      await expect(
        Pool1.connect(operator).depositRewards({ value: 0 }),
      ).to.be.revertedWith("ETH");
    });
    it("revert if no staked share", async () => {
      await expect(
        Pool1.connect(operator).depositRewards({ value: 10 }),
  ...
  describe("unstake", async () => {
    ...
    it("unstake", async () => {
      ...
      const user1TokenV2Reward = ethers.utils
        .parseEther("10")
        .mul(350)
        .div(36500);
      ...
      const expectedRewardPerShare = rewardsPerShareBefore.add(
        ethers.utils.parseUnits("10", 36).div(totalSharesDeposited),
      );

      expect(
        Number(
          user1TokenV2BalanceBefore
            .sub(user1TokenV2BalanceAfter)
            .sub(ethers.utils.parseEther("5")),
        ),
      ).to.be.lessThanOrEqual(Number(user1TokenV2Reward));
      expect(rewardsPerShareAfter).to.be.eq(expectedRewardPerShare);
    });

    it("revert if before lock time", async () => {
      await expect(
        Pool1.connect(user1).unstake(ethers.utils.parseEther("5")),
      ).to.be.revertedWith("REM: timelock");
    });
    it("revert if invalid unstake amount", async () => {
      await increaseBlockTimestamp(provider, 86400 * 14);
      await expect(
        Pool1.connect(user1).unstake(ethers.utils.parseEther("20")),
      ).to.be.revertedWith("REM: amount");
    });
  });
  describe("claimReward", async () => {
    ...
    it("compound claimed reward - eth", async () => {
      ...
      expect(
        Number(poolTokenV2BalanceAfter.sub(poolTokenV2BalanceBefore)),
      ).to.be.greaterThan(0);
      expect(
        shareInfoUser1After.amount.sub(shareInfoUser1Before.amount),
      ).to.be.eq(poolTokenV2BalanceAfter.sub(poolTokenV2BalanceBefore));
    });
    it("revert claimRewardAdmin if not Admin", async () => {
      await expect(
        Pool1.connect(user2).claimRewardAdmin(user1.address, false, 0),
      ).to.be.reverted;
    });
  });
  ...
  describe("migrate", async () => {
    ...
    it("revert if already migrated", async () => {
      await Pool1.migrate(user1.address);
      await expect(Pool1.migrate(user1.address)).to.be.revertedWith(
        "Already migrated!",
      );
    });
  ...
  describe("admin configuration role", async () => {
    ...
    it("revert if not admin", async () => {
      await expect(Pool1.connect(user1).setPoolExtension(operator.address)).to
        .be.reverted;
      await expect(Pool1.connect(user1).setStakingV1(operator.address)).to.be
        .reverted;
      await expect(Pool1.connect(user1).setLockupPeriod(86400 * 35)).to.be
        .reverted;
    });
