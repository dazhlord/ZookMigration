...
contract ZookV2 is IERC20, Initializable, UUPSUpgradeable, ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    Fees public _taxRates = Fees({buyFee: 0, sellFee: 1000, transferFee: 0});
    ...
    function distributeReward() external onlyOwner {
        require(lastRewardTime + 7 days <= block.timestamp, "it's too early");

        if (totalTokenHoldingForReward != 0 && _tOwned[rewardManager] != 0) {
             currentRewardPerToken += _tOwned[rewardManager] / totalTokenHoldingForReward;
        }
    ...
    function migration(address user, uint256 amount) external onlyOwner{
        require(isMigration, "Migration Disabled");
        require(IERC20(tokenV1).balanceOf(user) >= amount, "invalid input amount");
    ...
    function setRewardRatio(uint256 _newRatio) external onlyOwner {
        require(_newRatio>= 2500 && _newRatio <= 5000, "invalid ratio setting");
    ...
    function _finalizeTransfer(
        ...
        if(checkAvailableUserForReward(from)) updateTotalHoldingForReward(_tOwned[from] + amount, _tOwned[from]);
        if(checkAvailableUserForReward(to)) updateTotalHoldingForReward(_tOwned[to] - amountReceived, _tOwned[to]);
    ...
    function _updateReward(address user) internal {
        if(_tOwned[user] >= tokenLimitForReward)
            rewardBalance[user] += (currentRewardPerToken - lastRewardPerToken[user]) * _tOwned[user];
