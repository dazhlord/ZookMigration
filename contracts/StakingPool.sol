...
contract StakingPool is Context, Ownable, ReentrancyGuard {
    ...
    function migrate() external nonReentrant {
        require(
            address(stakingV1) != address(0),
            "Invalid staking v1 contract"
        );
        require(!isMigrated[_msgSender()], "Already migrated!");

        uint128 userStakeId = stakingV1.userStakeId(_msgSender());

        for (uint128 index = 0; index < userStakeId; index++) {
            (, , uint128 stakedAmount, bool claimed) = stakingV1.userMapping(
                _msgSender(),
                index
            );
            ...
            if (!claimed) {
                ...
                _setShare(_msgSender(), stakedAmount, false);
            }
        }
